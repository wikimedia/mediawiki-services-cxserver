	...
	for ( i = 0; i < len; i++ ) {
		...
		for ( j = 0; j < substringWordsLength; j++ ) {
			...
			if ( this.isApproximateEqual( word, substringNGrams[ j ] ) ) {
				...
				// el cual tiene un editor ejecutivo sobre las páginas noticiosas y un
				// editor de página del editorial encima páginas de opinión.],
				// [editor ejecutivo] will be the match till here. We do not ignore that match,
				// but will look for better  matches.
				break;
			}
		}
