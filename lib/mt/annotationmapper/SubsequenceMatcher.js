...
class SubSequenceMatcher {
	...
	*/
	...
	findFuzzyMatch( text, substring ) {
		...
		let substringNGrams = this.getWords( substring );
		let substringWordsLength = substringNGrams.length;
		let textNGrams = this.getNGrams( text, substringNGrams.length );
		let len = textNGrams.length;
		for ( let i = 0; i < len; i++ ) {
			let match = null;
			for ( let j = 0; j < substringWordsLength; j++ ) {
				let word = textNGrams[ i ][ j ];
				if ( this.isApproximateEqual( word, substringNGrams[ j ] ) ) {
					match = !match ? word : match + ' ' + word;
				} else {
					// The match sequence broke.
					// Example:
					// Sarching [editor de página del editorial] in [the new york times,
