...
class Google extends MTClient {
	...
	translateHtml( sourceLang, targetLang, sourceHtml ) {
		...
		if ( sourceHtml.length > 5000 ) {
			// Max limit is 5K characters for Google.
			return Promise.reject( new Error( 'Source content too long' ) );
		}
		...
		return preq.post( postData ).then( ( response ) => {
			this.metrics.increment( 'translate.Google.charcount', sourceHtml.length );
			return response.body.data.translations[ 0 ].translatedText;
		} ).catch( ( response ) => {
			throw new Error( `Translation with Google failed. Error: ${response.body.error.code} :
