...
class Google extends MTClient {
	...
	async translateHtml( sourceLang, targetLang, sourceHtml ) {
		...
		try {
			...
			if ( statusCode !== 200 ) {
				throw new Error(
					`Translation with Google failed. Error: ${ statusCode } for ${ sourceLang } > ${ targetLang }`
				);
			}
			const translatedData = await body.json();
			...
			throw new Error( `Translation with Google for ${ sourceLang } > ${ targetLang } failed: ${ error.message }` );
		}
	}
