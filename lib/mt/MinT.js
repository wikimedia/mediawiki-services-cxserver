...
class MinT extends MTClient {
	...
	async translate( sourceLang, targetLang, content, format = 'html' ) {
		...
		try {
			const { statusCode, body } = await request( this.conf.mt.MinT.api, {
				method: 'POST',
				headers: {
			...
			if ( statusCode !== 200 ) {
				...
				throw new Error( `MinT API Error ${ statusCode }:  ${ errorBody }` );
			}
			...
			const translationResponse = await body.json();
			return translationResponse.translation;
		} catch ( error ) {
			throw new Error( `Translation with MinT for ${ sourceLang } > ${ targetLang } failed: ${ error.message }` );
		}
	}
