...
class Elia extends MTClient {
	...
	async translateText( sourceLang, targetLang, sourceText ) {
		...
		if ( length > limit ) {
			return Promise.reject(
				new HTTPError( {
					status: 413,
					type: 'mt_error',
					detail: `Elia: Source content too long: ${ length } (${ limit } is the character limit)`
				} )
			);
		}
		...
		try {
			...
			throw new Error( `Translation with Elia for ${ sourceLang } > ${ targetLang } failed: ${ error.message }` );
		}
	}
