...
class Elia extends MTClient {
	 ...
	 * Elia supports HTML translation, but the precision of inline tags is not
	 * perfect to depend on it.
	...
	translateText( sourceLang, targetLang, sourceText ) {
		...
		const limit = 10000;
		if ( length > limit ) {
			return Promise.reject( new Error( `Source content too long: ${length} (${limit} is the limit)` ) );
		}

		const postData = {
			uri: this.conf.mt.Elia.api,
		...
		return preq.post( postData )
			.then( ( response ) => {
				...
				return response.body.translated_text;
			} )
			.catch( ( response ) => {
				throw new Error( `Translation with Elia failed. Error: ${this.getErrorName( response.status )} for ${sourceLang} > ${targetLang}` );
			} );
	}
