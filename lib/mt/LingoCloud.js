	...
	async translateText( sourceLang, targetLang, sourceText ) {
		...
		if ( length > limit ) {
			return Promise.reject(
				new Error( `LingoCloud: Source text too long: ${ length } (${ limit } is the limit)` )
			);
		}
		...
		if ( this.conf.proxy ) {
			options.proxyAgent = new ProxyAgent( this.conf.proxy, {
				requestTls: {
					// Ignore proxy certificate errors, if any.
					rejectUnauthorized: false
				}
			} );
			setGlobalDispatcher( options.proxyAgent );
		}
