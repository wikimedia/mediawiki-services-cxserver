...
class LingoCloud extends MTClient {
	...
	translateText( sourceLang, targetLang, sourceText ) {
		...
		key = this.conf.mt.LingoCloud.key;
		if ( key === null ) {
			return Promise.reject( new Error( 'LingoCloud service is misconfigured' ) );
		}

		if ( sourceText.length > 10000 ) {
			return Promise.reject( new Error( `Source text too long: ${sourceLang} - ${targetLang}` ) );
		}
		...
		return preq.post( postData )
			.then( ( response ) => {
				...
				this.metrics.increment( 'translate.LingoCloud.charcount', sourceText.length );
				if ( 'target' in body ) {
					let target = response.body.target;
