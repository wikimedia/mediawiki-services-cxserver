...
class MTClient {
	...
	translateReducedHtml( sourceLang, targetLang, sourceHtml ) {
		...
		if ( LinearDoc.Utils.isBlockTemplate( this.sourceDoc ) ) {
			// Do not translate block templates.
		...
		return this.translateHtml(
			...
			let translatedDoc = parser.builder.doc;
			...
			if ( translatedDoc.getRootItem().item.name !== this.sourceDoc.getRootItem().item.name ) {
				throw new Error(
	...
	translateItemDeferred( item, sourceLang, targetLang ) {
		...
		return this.translateTextWithTagOffsets(
			...
			newTextBlock = item.item.translateTags( translated.text, translated.rangeMappings );
	...
	translateHtml( sourceLang, targetLang, sourceHtml ) {
		...
		for ( let i = 0, len = this.sourceDoc.items.length; i < len; i++ ) {
			let item = this.sourceDoc.items[ i ];
			let about = cxUtil.getProp( [ 'item', 'attributes', 'about' ], item );
			let tagName = cxUtil.getProp( [ 'item', 'name' ], item );
			...
			if ( nonTranslatableTags.includes( tagName ) ) {
	...
	sanitize( html ) {
		...
		const rdfaAttrs = [ 'about', 'rel', 'resource', 'property', 'content', 'datatype', 'typeof', 'srcset' ];
	...
	translateTextWithTagOffsets( sourceLang, targetLang, sourceText, tagOffsets ) {
		var subSequences, sourceLines, preSpaces, postSpaces, trimmedSourceLines;
		...
		return this.translateLines(
			...
			let targetLines, targetText, rangeMappings;
	...
	isOverlappingRange( range, rangeArray ) {
		let rangeStart, rangeEnd, start, end;
