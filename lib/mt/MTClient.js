...
class MTClient {
	...
	translateHtml( sourceLang, targetLang, sourceHtml ) {
		...
		for ( let i = 0, len = this.sourceDoc.items.length; i < len; i++ ) {
			chain.push( this.translateItemDeferred( this.sourceDoc.items[ i ], sourceLang, targetLang ) );
		}
	...
	sanitize( html ) {
		...
		return this.DOMPurify.sanitize( html, {
			ADD_ATTR: [ 'typeof' ], // typeof is not a known attribute for DOMPurify
			ADD_URI_SAFE_ATTR: [ 'rel', 'typeof' ] // Without this rel="mw:WikiLink" attributes will be removed.
		} );
	}
	...
	buildSourceDoc( sourceHtml ) {
		...
		let parser = new LinearDoc.Parser( {
