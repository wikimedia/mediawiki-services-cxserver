...
class MTClient {
	...
	translateHtml( sourceLang, targetLang, sourceHtml ) {
		...
		for ( let i = 0, len = this.sourceDoc.items.length; i < len; i++ ) {
			let about = cxUtil.getProp(
				[ 'item', 'attributes', 'about' ], this.sourceDoc.items[ i ]
			);
			if ( about && this.sourceDoc.items[ i ].type === 'open' ) {
				// Start of transclusion context
				transclusionContext = about;
			}
			if ( about === transclusionContext && this.sourceDoc.items[ i ].type === 'close' ) {
