...
import { getProp, isPlainText } from '../util.js';
import { Doc, MwContextualizer, Parser } from '../lineardoc/index.js';
import { isIgnorableBlock, isNonTranslatable } from '../lineardoc/Utils.js';
import SubSequenceMatcher from './annotationmapper/SubsequenceMatcher.js';
...
class MTClient {
	...
	translateReducedHtml( sourceLang, targetLang, sourceHtml ) {
		...
		if ( isIgnorableBlock( this.sourceDoc ) ) {
	...
	translateHtml( sourceLang, targetLang, sourceHtml ) {
		...
		let targetDoc = new Doc( this.sourceDoc.wrapperTag );
		...
		for ( let i = 0, len = this.sourceDoc.items.length; i < len; i++ ) {
			...
			const tag = item.item;
			const about = getProp( [ 'item', 'attributes', 'about' ], item );
	...
	translateTextWithTagOffsets( sourceLang, targetLang, sourceText, tagOffsets ) {
		...
		return this.translateLines(
			...
			try {
				...
				// without annotations.
				this.log( 'debug', 'Error while mapping annotations ' + ex.stack );
				rangeMappings = {};
			}
			return {
				text: targetText,
	 ...
	 * @return {Object[]} Array of subsequence information objects with start, length, and text properties
	 */
