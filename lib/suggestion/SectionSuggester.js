...
error } );
		...
		if ( this.sectionMappingDatabaseConf.type === 'sqlite' ) {
			const queryStr = `SELECT source_title, target_title, frequency
				...
				`;
			if ( !dbRead ) {
				dbRead = new DatabaseSync( this.sectionMappingDatabaseConf.database, { readOnly: true } );
			}
			const stmt = dbRead.prepare( queryStr );
			return stmt.all( sourceLanguage, targetLanguage, ...sourceSections );
		} else {
