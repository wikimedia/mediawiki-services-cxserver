...
class SectionSuggester {
	...
	async getSections( language, title ) {
		...
		const headerSections = metadata.toc.entries.filter( ( entry ) => entry.level === 1 ) || [];
		return headerSections.map( ( section ) => section.html );
	}
	...
	async getTargetTitle( sourceTitle, sourceLanguage, targetLanguage ) {
		const titlePairInfo = await this.api.titlePairRequest( sourceTitle, sourceLanguage, targetLanguage );
		return titlePairInfo && titlePairInfo.targetTitle;
	}
	 ...
	 * @param {string} targetLanguage Target language code
	 * @param {string} [targetTitle] Target title. If not passed, it will be calculated.
	...
	async getMissingSections( sourceLanguage, sourceTitle, targetLanguage, targetTitle ) {
		...
		targetTitle = targetTitle || await this.getTargetTitle( sourceTitle, sourceLanguage, targetLanguage );
		if ( !targetTitle ) {
		...
		for ( let i = 0; i < sourceSections.length; i++ ) {
			...
			for ( let i = 0; i < candidateTargetSectionTitles.length; i++ ) {
				if ( existingTargetSections.indexOf( candidateTargetSectionTitles[ i ] ) >= 0 ) {
					present[ sourceSectionTitle ] = candidateTargetSectionTitles[ i ];
