...
import MwApiRequest from '../mw/MwApiRequest.js';
...
class SourceSuggester {
	 ...
	 * @param {Function} config.mtProviderFactory A function that returns a Promise<MTClient>
	 *   instance
	 */
				...
				try {
					...
					if ( title ) {
						...
						try {
							...
							this.logger.log( 'error', `Error fetching title info for ${ title } in ${ language }: ${ fetchError.message }` );
						}
					}
				} catch ( titleGenError ) {
					this.logger.log( 'error', `Error generating title for language: ${ language }: ${ titleGenError.message }` );
				}
			} )
		);
	...
	async getWikidataQid( language, title ) {
		...
		return getProp( [ 'search', 0, 'id' ], searchResult );
	}
	...
	async getWikidataLabels( wikidataId, languages ) {
		...
		const labels = getProp( [ 'entities', wikidataId, 'labels' ], searchResult );
		if ( labels ) {
	 ...
	 * @param {string} language
	 * @param {string} title
	 * @return {string|undefined} The machine translated title or undefined if
	 * no MT client is found for that language pair
	 */
	async getMachineTranslatedTitle( language, title ) {
		const mtClient = await this.mtProviderFactory( this.targetLanguage, language );
		if ( mtClient ) {
...
export default SourceSuggester;
