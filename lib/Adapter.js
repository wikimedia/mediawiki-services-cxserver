...
import translationUnits from './translationunits/index.js';

class Adapter {
	constructor( sourceLanguage, targetLanguage, api, context ) {
		this.parser = new Parser( new MwContextualizer() );
		this.parser.init();
	...
	adapt( content ) {
		...
		return this.originalDoc.adapt( this.getAdapter.bind( this ) )
			.then( ( adaptedDoc ) => {
				...
				const sourcePolicy = registry.getReferencePunctuationPolicy( this.sourceLanguage );
				const targetPolicy = registry.getReferencePunctuationPolicy( this.targetLanguage );
				// Only reposition references when the languages disagree on where
				// the reference goes relative to punctuation. When they agree, the
				// source placement already follows the target convention.
				if ( targetPolicy && targetPolicy.punctuation.length &&
					sourcePolicy.policy !== targetPolicy.policy ) {
					return adaptedDoc.adaptReferencePunctuation( targetPolicy );
				}
				return adaptedDoc;
			} );
	}
