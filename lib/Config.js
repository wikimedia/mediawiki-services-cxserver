...
class CXConfig {
	constructor( app ) {
		...
		this.languages = null;
		this.mt = {};
		this.referencePunctuation = null;
	}
	...
	async parseAndLoadConfig() {
		...
		// Reference position relative to punctuation
		this.loadReferencePunctuationConf();
	}

	loadReferencePunctuationConf() {
		const conf = this.app.conf.referencePunctuation;
		if ( this.isObject( conf ) ) {
			this.referencePunctuation = conf;
		} else if ( this.isString( conf ) ) {
			try {
				this.referencePunctuation = load( readFileSync( conf ) );
			} catch ( e ) {
				this.log( 'warn', 'Could not load the reference punctuation config: ' + e );
			}
		}
	}

	/**
	 * Get the reference position policy for the given target language.
	 *
	 * @param {string} language Target language code
	 * @return {Object|null} { policy: 'before'|'after', punctuation: string[] }
	 *   or null when the feature is not configured.
	 */
	getReferencePunctuationPolicy( language ) {
		if ( !this.referencePunctuation ) {
			return null;
		}
		const before = this.referencePunctuation.before || [];
		return {
			policy: before.includes( language ) ? 'before' : 'after',
			punctuation: this.referencePunctuation.punctuation || []
		};
	}
