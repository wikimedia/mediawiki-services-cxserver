...
class CXConfig {
	...
	loadServiceConf( input, type ) {
		...
		for ( const providerName of providers ) {
			...
			if ( this.isObject( languageRef ) ) {
				...
				try {
					const providerLanguages = yaml.safeLoad( fs.readFileSync( languageRef ) );
	...
	getToolSetForLanguage( language ) {
		const tools = { mt: this.mt, dictionary: this.dictionary },
			result = { mt: {}, dictionary: {} };
		...
		Object.keys( tools ).forEach( ( toolName ) => {
			...
			Object.keys( tool.defaults || {} )
				...
				.forEach( ( pair ) => {
					const targetLanguage = pair.substring( language.length + 1 );
			...
			for ( let i = 0, length = providers.length; i < length; i++ ) {
				...
				for ( let j = 0, langsLength = languages.length; j < langsLength; j++ ) {
					...
					} else if ( results.indexOf( provider ) < 0 ) { // Don't duplicate providers
