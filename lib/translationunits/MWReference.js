...
class MWReference extends TranslationUnit {
	...
	async adapt() {
		...
		// VE clipboard mode adds a rendered preview of the reference marked
		// with data-ve-ignore. The client discards it on parse, so adapting
		// it is wasted work. Remove such elements before adaptation.
		if ( refBody.includes( 'data-ve-ignore' ) ) {
			const bodyContainer = JSDOM.fragment( `<div>${ refBody }</div>` ).firstElementChild;
			const ignoredNodes = bodyContainer.querySelectorAll( '[data-ve-ignore]' );
			if ( ignoredNodes.length ) {
				ignoredNodes.forEach( ( node ) => node.remove() );
				// Keep the original body if nothing remains after the removal.
				// A reference must not lose its body because of this cleanup.
				if ( bodyContainer.innerHTML.trim() ) {
					refBody = bodyContainer.innerHTML;
				}
			}
		}
		...
		if ( !isTransclusion && this.context.mtClient ) {
		...
		if ( isTransclusion ) {
			...
			const translatedRefBodyDom = new JSDOM( translatedRefBody );
