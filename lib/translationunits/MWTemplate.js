...
MWTemplate.prototype.adapt = cxutil.async( function* () {
	...
	// large objects. As of now, CX2 frontend does not need that data since we use VE template editor.
	// When we use custom template editor, this information may be needed, but till then, don't send
	// unused data in API result.
	dataCX = dataCX.map( ( tranclusion ) => ( tranclusion && { adapted: tranclusion.adapted } ) );
