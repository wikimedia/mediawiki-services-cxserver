...
class MWGallery extends TranslationUnit {
	...
	async adapt() {
		...
		for ( let i = 0; i < imageItems.length; i++ ) {
			...
			namespaceAlias = await this.api.getNamespaceAlias( 'File', this.targetLanguage );
			...
			// Convert the current image caption to html
				imageCaptionHTML = await new MWApiRequest( {
				...
				imageCaptionHTMLTranslated = await this.context.conf.mtClient.translate(
