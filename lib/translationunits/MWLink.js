...
function isISBN( node ) {
	const isbnLinkRegex = new RegExp( '^\\./Special:BookSources/([0-9Xx]+)' );
...
class MWLink extends TranslationUnit {
	async adapt() {
		...
		if ( !this.node.attributes.title ) {
			...
			this.log( 'warn', 'No title for the MWLink: ' + JSON.stringify( this.node ) );
		}
		...
		if ( targetTitle ) {
			...
			const wikidataLabel = await wikidataLabelRequest;
			const translatedLabel = await translatedLabelRequest;
