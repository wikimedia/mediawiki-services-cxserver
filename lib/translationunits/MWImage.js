...
class MWImage extends TranslationUnit {
	constructor( node, sourceLanguage, targetLanguage, api, context ) {
		...
		this.isInlineImage = node.name === 'figure-inline' || node.name === 'span';
	}
	...
	async adapt() {
		...
		if ( this.isCommonsImage( sourceImage.attributes.src ) ) {
			...
			const targetResource = this.sourceResource.replace( /^(\.\.?\/)*([^:]+)(:)/, '$1' + namespaceAlias + '$3' );
...
MWImage.matchTagNames = [ 'figure', 'figure-inline', 'span' ];
