...
class MWImage extends TranslationUnit {
	...
	isInlineImage() {
		...
		// TODO: Rename this adapter to a generic MWFile since it can handle images and audio too.
		return this.node.name === 'span' && this.node.attributes.typeof &&
			this.node.attributes.typeof.match( /(^|\s)(mw:File|mw:Image|mw:Video|mw:Audio)\b/ );
	}
	...
	async adapt() {
		...
		if ( !sourceImage ) {
			throw new Error( 'img tag not found in the figure with mw:File/Thumb for id: ' + this.node.attributes.id );
		}
...
MWImage.matchTagNames = [
	'figure', // Figure can also mean Video or Audio too. https://www.mediawiki.org/wiki/Specs/HTML/2.4.0#Audio/Video
	'span'
];
