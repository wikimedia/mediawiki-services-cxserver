...
class PrometheusClient {
	...
	makeMetric( options ) {
		...
		const labelNames = [ normalisedName ];
		if ( options.labels?.names ) {
			labelNames.concat( options.labels.names );
		}
		...
		metric.timing = metric.observe;
		metric.endTiming = ( startTime, labelValues ) => {
			metric.timing( ( Date.now() - startTime ) / 1000, labelValues );
		};
