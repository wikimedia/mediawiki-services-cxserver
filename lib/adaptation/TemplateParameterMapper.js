...
class TemplateParameterMapper {
	...
	getParameterMap() {
		...
		for ( let name in this.sourceParams ) {
			...
			let sourceTemplateParam = this.sourceTemplateData.params[ name ] ||
			...
			if ( !sourceTemplateParam ) {
				...
				let paramMatchingAlias = Object.keys( this.sourceTemplateData.params ).find(
			...
			for ( let paramName in this.targetTemplateData.params ) {
				...
				let match = ( [ ...targetKeyAndAliases ].some( ( key ) => {
