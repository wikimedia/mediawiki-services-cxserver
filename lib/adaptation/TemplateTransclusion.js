...
import TemplateParameterMapper from './TemplateParameterMapper.js';

const TEMPLATE_MAPPING_THRESHOLD_SCORE = 0.56;

// Lazily opened, reused for the process lifetime.
...
class TemplateTransclusion {
	...
	async adapt() {
		...
		const normalizedSourceTitle = await this.api.normalizeTitle(
			sourceTitle,
			this.sourceLanguage
		);
		try {
			targetInfo = await this.api.titlePairRequest(
				sourceTitle,
				this.sourceLanguage,
				this.targetLanguage
			);
		...
		if ( !targetTitle ) {
			...
			const templateNamespaceAlias = await this.api.getNamespaceAlias(
				'Template',
				this.targetLanguage
			);
			...
			try {
				sourceTemplateData = await this.api.templateDataRequest(
					sourceTitle,
					this.sourceLanguage
				);
			} catch {
			...
			try {
				...
				targetTemplateData = await this.api.templateDataRequest(
					targetTitle,
					this.targetLanguage,
					paramHints
				);
			...
			if ( this.templateMappingDatabase ) {
				mappingFromDatabase = await this.getMappingFromDatabase(
					this.templateMappingDatabase,
			...
			if ( Object.keys( sourceParams ).length > 0 ) {
				const mappedParamNames = Object.keys(
					adaptedTransclusionDef.template.params
				);
				metadata.adapted = mappedParamNames.length > 0;
				const isEveryMandatoryParamMapped = mandatoryParams.every( ( paramName ) => mappedParamNames.includes( paramName )
				);
	...
	async getMappingFromDatabase(
		templateMappingDatabase,
		templateName,
		threshold = TEMPLATE_MAPPING_THRESHOLD_SCORE
	) {
		if ( !db ) {
			db = new DatabaseSync( templateMappingDatabase, { readOnly: true } );
		}
		...
		for ( const row of results ) {
			mapping[ row.source_param ] = row.target_param;
		}
		return mapping;
	}
}

export default TemplateTransclusion;
