...
const contentBranchNodeNames = [ 'blockquote', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'p', 'pre', 'div', 'table', 'ol', 'ul', 'dl', 'figure', 'center' ];
...
class MwContextualizer extends Contextualizer {
	...
	getChildContext( tag ) {
		...
		if ( tag.name === 'figure' || tag.name === 'figure-inline' ) {
			return 'media';
		}
	 ...
	 */
	isRemovable( tag ) {
		let removableSections = this.config.removableSections;
		...
		for ( let i = 0; i < removableSections.rdfa.length; i++ ) {
			if ( rdfa.includes( removableSections.rdfa[ i ] ) ) {
				if ( tag.attributes.about ) {
					this.removableTransclusionFragments.push( tag.attributes.about );
				}
				return true;
		...
		let dataMW = tag.attributes[ 'data-mw' ];
		if ( !dataMW ) {
			return false;
		}
		let mwData = JSON.parse( dataMW );
		let templateName = cxutil.getProp( [ 'parts', 0, 'template', 'target', 'wt' ], mwData );
		if ( !templateName ) {
			return false;
		}
		...
		for ( let i = 0; i < removableSections.templates.length; i++ ) {
			...
			let removableTemplateName = removableSections.templates[ i ];
			...
			let match = removableTemplateNameRegExp ?
