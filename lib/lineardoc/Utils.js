...
function findAll( text, regex, callback ) {
	...
	while ( true ) {
		...
		let boundary = callback( text, match );
		if ( boundary !== null ) {
...
function isExternalLink( tag ) {
	return tag.name === 'a' && tag.attributes && tag.attributes.rel === 'mw:ExtLink';
}
...
function isInlineEmptyTag( tagName ) {
	...
	let inlineEmptyTags = [ 'br', 'img', 'source', 'track', 'link', 'meta' ];
...
function getChunkBoundaryGroups( boundaries, chunks, getLength ) {
	...
	for ( let i = 0, len = chunks.length; i < len; i++ ) {
		let groupBoundaries = [];
		let chunk = chunks[ i ];
		let chunkLength = getLength( chunk );
		while ( true ) {
			let boundary = boundaries[ boundaryPtr ];
...
function addCommonTag( textChunks, tag ) {
	...
	let commonTags = textChunks[ 0 ].tags.slice();
	for ( let i = 1, iLen = textChunks.length; i < iLen; i++ ) {
		let tags = textChunks[ i ].tags;
		let j, jLen;
	...
	for ( let i = 0, iLen = textChunks.length; i < iLen; i++ ) {
		...
		for ( let j = 0, jLen = tags.length; j < jLen; j++ ) {
			...
			if (
				...
				tag.attributes.rel === 'mw:WikiLink' &&
...
/**
 * Check if the passed document is a section containing block level template
 * @param {Doc} sectionDoc
 * @return {boolean}
 */
function isBlockTemplate( sectionDoc ) {
	let blockStack = [];
	...
	for ( let i = 1, len = sectionDoc.items.length; i < len; i++ ) {
		let item = sectionDoc.items[ i ];
		let tag = item.item;
		let type = item.type;

		if ( type === 'open' ) {
			blockStack.push( tag );
			if ( !firstBlockTemplate && isTransclusion( tag ) ) {
				firstBlockTemplate = tag;
			}
		}
		if ( type === 'close' ) {
			let currentCloseTag = blockStack.pop();
			if ( currentCloseTag &&
				blockStack.length === 0 &&
