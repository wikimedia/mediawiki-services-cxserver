...
class Builder {
	...
	isCategory( tag ) {
		return tag.name === 'link' && tag.attributes.rel === 'mw:PageProp/Category' &&
	...
	popInlineAnnotationTag( tagName ) {
		...
		for ( i = this.textChunks.length - 1; i >= 0; i-- ) {
			...
			let chunkTag = textChunk.tags[ textChunk.tags.length - 1 ];
		...
		if ( replace && ( Utils.isReference( tag ) || Utils.isExternalLink( tag ) ) ) {
	 ...
	 * @param {boolean} isTransclusionContext
	 */
	addInlineContent( content, canSegment, isTransclusionContext ) {
		// If the content is a category tag, capture it separately and don't add to doc.
		// Categories under the transclusion are not added.
		if ( !isTransclusionContext && this.isCategory( content ) ) {
	...
	finishTextBlock() {
		var whitespace = [], whitespaceOnly = true;
