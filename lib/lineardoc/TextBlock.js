...
class TextBlock {
	 ...
	 */
	getTagOffsets() {
		var textBlock = this, commonTags = this.getCommonTags();
	...
	translateTags( targetText, rangeMappings ) {
		var i, iLen, j, rangeMapping, sourceTextChunk, text, pos, textChunk,
			offset, sourceRangeEnd, targetRangeEnd, tail, tailSpace, commonTags,
			// map of { offset: x, textChunks: [...] }
			emptyTextChunks = {}, emptyTextChunkOffsets = [],
		...
		for ( i = 0, iLen = rangeMappings.length; i < iLen; i++ ) {
			...
			textChunks.push( {
				...
				textChunk: new TextChunk( text, sourceTextChunk.tags, sourceTextChunk.inlineContent )
			} );
		...
		for ( i = 0, iLen = textChunks.length; i < iLen; i++ ) {
			...
			if ( textChunk.start < pos ) {
				...
				textChunks.splice( i, 0, {
					...
					textChunk: new TextChunk( targetText.substr( pos, textChunk.start - pos ), commonTags )
				} );
				i++;
				iLen++;
			}
     ...
     */
	getHtml() {
		var i, iLen, j, jLen, textChunk, matchTop, oldTags, html = [];
	 ...
	 */
	getTagForId() {
		var i, iLen, j, jLen, textChunk;

		for ( i = 0, iLen = this.textChunks.length; i < iLen; i++ ) {
			textChunk = this.textChunks[ i ];

			for ( j = 0, jLen = textChunk.tags.length; j < jLen; j++ ) {
				if ( textChunk.tags[ j ] ) {
	...
	segment( getBoundaries, getNextId ) {
		var allTextChunks, currentTextChunks, groups, i, iLen, group, offset, textChunk,
			j, jLen, leftPart, rightPart, boundaries, relOffset;
		...
		groups = Utils.getChunkBoundaryGroups( getBoundaries( this.getPlainText() ), this.textChunks, function ( textChunk ) {
		...
		for ( i = 0, iLen = groups.length; i < iLen; i++ ) {
			...
			for ( j = 0, jLen = boundaries.length; j < jLen; j++ ) {
				...
				if ( relOffset === 0 ) {
					...
					leftPart = new TextChunk( textChunk.text.substring( 0, relOffset ), textChunk.tags.slice() );
					rightPart = new TextChunk( textChunk.text.substring( relOffset ), textChunk.tags.slice(), textChunk.inlineContent );
	...
	adapt( getAdapter ) {
		...
		this.textChunks.forEach( ( chunk ) => {
			var tagPromises = [], adapter, adaptPromise, tags = chunk.tags;
			tags.forEach( ( tag ) => {
	 ...
	 */
	dumpXmlArray( pad ) {
		var i, len, chunk, tagsDump, tagsAttr, dump = [];
		for ( i = 0, len = this.textChunks.length; i < len; i++ ) {
