...
class Doc {
	...
	segment( getBoundaries ) {
		...
		for ( let i = 0, len = this.items.length; i < len; i++ ) {
			...
			if ( this.items[ i ].type === 'open' ) {
				...
				// We can use a more strict test for typeof, data-mw attributes but checking for about
				// attributes also serve the purpose.
	...
	wrapSections() {
		...
		for ( let i = 0; i < itemsLength; i++ ) {
			...
			if ( type === 'open' ) {
				...
				const textBlock = item.item;
				const tag = textBlock.getTagForId();
				...
				const isConnected = tag && prevSection === getTagId( tag );
				...
				if ( !currSection ) {
					...
					currSection = getTagId( tag );
					if ( !currSection ) {
						throw new Error( `No id for the opened section for tag ${tag.name}` );
					}
	...
	reduce( idCounter ) {
		...
		const isNonTranslatable = ( tag ) => {
			const nonTranslatableTags = [ 'style', 'svg', 'script' ];
			const nonTranslatableRdfa = [ 'mw:Entity' ];
			return nonTranslatableTags.includes( tag.name ) ||
				( tag.attributes && (
					nonTranslatableRdfa.includes( tag.attributes.typeof ) ||
					nonTranslatableRdfa.includes( tag.attributes.rel ) )
				);
		};
