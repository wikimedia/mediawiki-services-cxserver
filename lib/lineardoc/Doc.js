...
class Doc {
	 ...
	 * @param {number} [attrDumpIdSeq] The id sequence start value to use for the attribute dump
	...
	reduce( attrDumpIdSeq ) {
		...
		if ( this.wrapperTag && hasAttributesToSave( this.wrapperTag ) ) {
			id++;
			attrDump[ id ] = Object.assign( {}, this.wrapperTag.attributes );
			this.wrapperTag.attributes = { id };
		}
		for ( let i = 0, len = this.items.length; i < len; i++ ) {
			let item = this.items[ i ];
			let tag = item.item;
			...
			if ( type === 'open' ) {
				if ( hasAttributesToSave( tag ) ) {
					id++;
					attrDump[ id ] = Object.assign( {}, tag.attributes ); // Shallow copy
					tag.attributes = { id };
				}
			...
			for ( let j = 0, len = textblock.textChunks.length; j < len; j++ ) {
				...
				if ( chunk.tags ) {
					for ( let k = 0, len = chunk.tags.length; k < len; k++ ) {
						...
						id++;
						attrDump[ id ] = Object.assign( {}, tag.attributes );
						tag.attributes = { id };
					}
				}
				...
				if ( chunk.inlineContent ) {
					if ( chunk.inlineContent.reduce ) {
						let inlineReduceResult = chunk.inlineContent.reduce( id );
						...
						id++;
						attrDump[ id ] = Object.assign( {}, chunk.inlineContent.attributes );
						chunk.inlineContent.attributes = { id };
					}
				}
			}
			reducedDoc.addItem( type, tag );
