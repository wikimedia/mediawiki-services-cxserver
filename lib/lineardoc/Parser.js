...
class Parser extends SAXParser {
	...
	init() {
		...
		// Whether the closing of section to be deferred if the context is transclusion.
		this.deferSectionClose = false;
	}
	...
	onopentag( tag ) {
		...
		if ( this.transclusionContext && this.transclusionContext !== tag.attributes.about ) {
			...
			// Close any deferred section wraps.
			if ( this.deferSectionClose ) {
	...
	onclosetag( tagName ) {
		...
		if ( Utils.isInlineEmptyTag( tagName ) ) {
			...
			if ( this.canSectionWrap( tag ) ) {
				if ( this.transclusionContext ) {
					// This is a section inside a transclusion context.
					// Defer closing till all fragments(if any) are included
					this.deferSectionClose = true;
				} else {
					this.builder.popBlockTag( 'section' );
				}
			}
