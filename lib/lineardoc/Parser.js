...
class Parser extends SAXParser {
	...
	onclosetag( tagName ) {
		...
		if ( Utils.isInlineEmptyTag( tagName ) ) {
			...
			if (
				Utils.isTransclusion( tag ) &&
				this.transclusionContext && tag.attributes.about === this.transclusionContext.about
			) {
				this.transclusionContext = null;
			}
	...
	isInlineAnnotationTag( tagName ) {
		...
		if ( tagName === 'span' && this.contextualizer.getContext() === 'media' ) {
			return false;
		}
