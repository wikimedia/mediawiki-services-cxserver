	...
	processQueue() {
		...
		function rejectSubqueue( rejectQueue ) {
			...
			for ( i = 0, len = rejectQueue.length; i < len; i++ ) {
				let error = new Error( 'Rejecting items in subqueue from BatchedAPIRequest. ' +
			...
			for ( let pageid in pages ) {
				...
				for ( let i in redirects ) {
					...
					if ( redirects[ i ].to === page.title ) {
						...
						// NOTE: It is important to set the value for all redirects in `processed`
						// array so that there are no pending Promises after
						// BatchedAPIRequest.prototype.set method call.
						// For example. If A->C, B->C, then we should not break after
						// finding and setting A. We should also set B in `processed` array.
						// rejectSubqueue will cause error if any pending promise is tried
						// to reject. See T199885
					}
				}
			}
...
BatchedAPIRequest.prototype.get = cxutil.async( function* ( title ) {
	...
	normalizedTitle = this.titleMap.get( title ) || ( yield this.normalizeTitle( title, this.sourceLanguage ) );
	if ( title !== normalizedTitle ) {
		this.titleMap.set( title, normalizedTitle );
	}

	if ( this.promises.has( normalizedTitle ) ) {
		return this.promises.get( normalizedTitle );
	}

	this.promises.set( normalizedTitle, new cxutil.Deferred() );
...
BatchedAPIRequest.prototype.set = cxutil.async( function* ( entries ) {
	...
	for ( title in entries ) {
		...
		normalizedTitle = this.titleMap.get( title ) ||
			( yield this.normalizeTitle( title, this.sourceLanguage ) );
		if ( !this.promises.has( normalizedTitle ) ) {
			this.promises.set( normalizedTitle, new cxutil.Deferred() );
		}
		this.promises.get( normalizedTitle ).resolve( entries[ title ] );
	}
} );
...
BatchedAPIRequest.prototype.dumpCache = cxutil.async( function* () {
	...
	for ( let [ title, value ] of this.promises.entries() ) {
		output[ title ] = yield value;
	}
