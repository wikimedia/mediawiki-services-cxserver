			...
			for ( let pageid in pages ) {
				...
				for ( let i in redirects ) {
					...
					if ( redirects[ i ].to === page.title ) {
						...
						// array so that there are no pending Promises after BatchedAPIRequest.prototype.set
						// method call. For example. If A->C, B->C, then we should not break after
						// finding and setting A. We should also set B in `processed` array.
						// rejectSubqueue will cause error if any pending promise is tried to reject.
						// See T199885
					}
				}
			}
