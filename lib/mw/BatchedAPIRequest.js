...
class BatchedAPIRequest extends MWApiRequest {
	...
	processQueue() {
		...
		function rejectSubqueue( rejectQueue, error ) {
			let i, len;
			if ( !error ) {
				error = new Error( 'Rejecting items in subqueue from BatchedAPIRequest. ' +
	...
	get( title ) {
		...
		this.promises.set( title, new Deferred() );
...
export default BatchedAPIRequest;
