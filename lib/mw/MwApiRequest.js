...
class MWApiRequest {
	 ...
	 * @cfg {Object} config.context Application context
	 * @cfg {string} config.sourceLanguage Source language
	 * @cfg {string} [config.targetLanguage] target language
	 */
	constructor( config ) {
	...
	htmlToWikiText( html, language ) {
		...
		const path = `${domain}/v3/transform/html/to/wikitext`;
	...
	wikitextToHTML( wikitext, language ) {
		...
		const path = `${domain}/v3/transform/wikitext/to/html`;
		return this.restApiGet( domain, path, restReq )
			.then( ( response ) => response.body );
	}
	...
	setupApiTemplates( app ) {
		...
		if ( !app.conf.restbase_req ) {
			app.conf.restbase_req = {
				...
				uri: 'http://{{domain}}/w/rest.php/{+path}',
				query: '{{ default(request.query, {}) }}',
