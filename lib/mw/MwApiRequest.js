...
// A single shared Agent for all MediaWiki API requests in this process.
//
// Previously, each mwGet/mwPost call created `new Agent(...)` inline. This meant every
// batched request opened its own connection pool to the same Wikipedia origin — no TCP
// connection reuse, no keep-alive, and a burst of parallel connections on every dispatch
// cycle. MediaWiki's rate limiter sees this as a flood of new connections and throttles.
//
// With a module-level agent, undici maintains a persistent connection pool per origin
// (e.g. en.wikipedia.org, ml.wikipedia.org). Subsequent requests to the same host reuse
// existing connections, dramatically reducing connection overhead and rate-limit pressure.
const mwApiAgent = new Agent( {
