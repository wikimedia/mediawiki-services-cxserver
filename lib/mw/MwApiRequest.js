...
class MWApiRequest {
	...
	async mwPost( domain, query ) {
		...
		const request = this.context.mwapi_tpl.expand( {
			request: {
				...
				headers: {
					'user-agent': this.context.user_agent,
	...
	setupApiTemplates( app ) {
		...
		if ( !this.context.mwapi_req ) {
			this.context.mwapi_req = {
		...
		this.context.mwapi_tpl = new Template( this.context.mwapi_req );
		...
		app.restbase_tpl = new Template( this.context.restbase_req );
