...
import SiteInfoRequest from './SiteInfoRequest.js';
import TemplateDataRequest from './TemplateDataRequest.js';
import TitleInfoRequest from './TitleInfoRequest.js';
import TitlePairRequest from './TitlePairRequest.js';
import WikidataRequest from './WikidataRequest.js';
import PageMetadataRequest from './PageMetadataRequest.js';
import SectionTitlesRequest from './SectionTitlesRequest.js';
import SectionTitlesAndTargetTitleRequest from './SectionTitlesAndTargetTitleRequest.js';
...
class MWApiRequestManager {
	 ...
	 *   target language
	 * @param {string} sourceLanguage Source language code
	 * @param {string} targetLanguage Source language code
	 * @param {boolean} includeSectionSizes Whether to include section sizes in response
	 * @return {Promise}
	 */
	sectionTitlesAndTargetTitleRequest( title, sourceLanguage, targetLanguage, includeSectionSizes = false ) {
		return this.normalizeTitle( title, sourceLanguage ).then( ( normalizedTitle ) => {
			const cacheKey = `sectiontitlesandtargettitle/${ sourceLanguage }/${ targetLanguage }`;
			const ctor = () => new SectionTitlesAndTargetTitleRequest(
				{ sourceLanguage, targetLanguage, includeSectionSizes, context: this.context }
			);
			return this.cachedBatchRequest( cacheKey, ctor, normalizedTitle );
		} );
	}
	 ...
	 * @return {Promise}
	 */
	sectionTitlesRequest( title, sourceLanguage ) {
		return this.normalizeTitle( title, sourceLanguage ).then( ( normalizedTitle ) => {
			const cacheKey = `sectiontitles/${ sourceLanguage }`;
			const ctor = () => new SectionTitlesRequest(
				{ sourceLanguage, context: this.context }
			);
			return this.cachedBatchRequest( cacheKey, ctor, normalizedTitle );
		} );
	}
...
export default MWApiRequestManager;
