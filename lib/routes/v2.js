...
import { HTTPError, assertValidLanguageCodes } from '../util.js';
...
router.get( '/suggest/title/:title/:from/:to/:provider?', suggestTargetTitle );
...
router.get( '/suggest/sections/:from/:to', fetchMultipleSectionSuggestions );
...
async function suggestTargetTitle( req, res, next ) {
	...
	if ( !assertValidLanguageCodes( res, sourceLanguage, targetLanguage ) ) {
		return;
	}

	if ( !title ) {
		return res.status( 400 ).end( 'Source page title should be provided' );
	}
...
async function suggestSectionsBySourceSectionTitles( req, res, next ) {
	...
	try {
		...
		const titles = await new SectionSuggester(
			api, req.app.conf.sectionmapping, req.app.dbPool
...
async function translate( req, res, next ) {
	...
	if ( provider ) {
		try {
			...
			if ( !( e instanceof MTClientError ) || e.status >= 500 ) {
	...
	// It's possible to specify a custom provider that supports custom language codes.
	// So check if an MT client was found, if not validate the language codes.
	if ( !mtClient && !assertValidLanguageCodes( res, from, to ) ) {
		return;
	}
	...
	if ( !sourceHtml || sourceHtml.trim().length === 0 ) {
		return next( Object.assign( new Error( 'Content for translate is not given or is empty' ), { status: 400 } ) );
	}
...
async function suggestSections( req, res, next ) {
	...
	try {
		...
		const sectionSuggester = new SectionSuggester( api, req.app.conf.sectionmapping, req.app.dbPool );
		const [ sourceSections, existingTargetSections ] = await Promise.all( [
			sectionSuggester.getSections( sourceLanguage, sourceTitle ),
			sectionSuggester.getSections( targetLanguage, targetTitle )
		] );
		const { present, missing } = await sectionSuggester.getPresentAndMissingSections(
			sourceLanguage,
			sourceTitle,
			targetLanguage,
			sourceSections,
		...
		if ( includeSectionSizes ) {
			response.sourceSectionSizes = sectionSuggester.getSectionSizes( sourceLanguage, sourceTitle );
		}

		return res.json( { sections: response } );
	} catch ( error ) {
...
/**
 * Suggest missing sections in target article based on the sections present in corresponding source article,
 * for an array of given page titles. The suggested sections are suggestions only and does not guarantee 100% accuracy.
 *
 * @param {Request} req Request object
 * @param {Response} res Response object
 * @param {Function} next Next middleware
 * @return {Promise}
 */
async function fetchMultipleSectionSuggestions( req, res, next ) {
	const sourceLanguage = req.params.from;
	const targetLanguage = req.params.to;
	...
	const sourceTitles = req.query.titles?.split( '|' ) || [];

	if ( sourceTitles.length === 0 ) {
		return res.status( 400 ).end( 'No valid source titles provided' );
	}
	...
	try {
		...
		const results = await Promise.allSettled(
			sourceTitles.map( async ( sourceTitle ) => {
				const { targetTitle, sectionTitles, sectionSizes } = await api.sectionTitlesAndTargetTitleRequest(
					sourceTitle,
					sourceLanguage,
					targetLanguage,
					includeSectionSizes
				...
				if ( !targetTitle ) {
					throw new Error( `Target article does not exist for ${ sourceLanguage }:${ sourceTitle } in ${ targetLanguage } language` );
				}

				const targetSections = await api.sectionTitlesRequest( targetTitle, targetLanguage );
				const sectionMappingDbConf = req.app.conf.sectionmapping;
				const dbPool = req.app.dbPool;

				/** @type {{present: Object, missing: Object, sourceSectionSizes: (Object|undefined)}} */
				const { present, missing } = await new SectionSuggester( api, sectionMappingDbConf, dbPool ).getPresentAndMissingSections(
					sourceLanguage,
					sourceTitle,
					targetLanguage,
					sectionTitles,
				...
				if ( includeSectionSizes ) {
					suggestion.sourceSectionSizes = sectionSizes;
				}
		...
		// Format response with successful and failed results
		const response = {
			results: results.map( ( result, index ) => {
				if ( result.status === 'fulfilled' ) {
					...
					return {
						sourceTitle: sourceTitles[ index ],
		...
		req.app.logger.error( `Error suggesting sections for titles "${ sourceTitles.join( '|' ) }" in language "${ sourceLanguage }" to "${ targetLanguage }": ${ error.stack }` );
		next( error );
	}
}
