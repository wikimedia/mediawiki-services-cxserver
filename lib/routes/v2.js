...
const MWPageLoader = require( '../mw/MWPageLoader' );
const jwt = require( 'jsonwebtoken' );

const MWApiRequestManager = require( '../mw/MWApiRequestManager' );
const { SourceSuggester, SectionSuggester } = require( '../suggestion' );
const express = require( 'express' );
const router = express.Router();
const v1Routes = require( './v1' );

router.get( '/', v1Routes.getSpec );
router.post( '/mt/:from/:to/:provider?', v1Routes.machineTranslate );
router.get( '/list/pair/:from/:to', v1Routes.listToolForLanguagePair );
router.get( '/list/languagepairs', v1Routes.listLanguagePairs );
router.get( '/list/:tool/:from?/:to?', v1Routes.listToolForLanguagePairsAndTool );
 ...
 * @return {Promise}
 */
async function suggestTargetTitle( req, res ) {
	...
	try {
		...
		const mtClient = v1Routes.getMTClient( req, res );
 ...
 */
async function suggestSectionsBySourceSectionTitles( req, res ) {
...
function fetchPage( req, res ) {
	...
	return pageLoader.getPage( title, revision, true /* wrapSections */ ).then(
		...
		( error ) => {
			req.app.logger.error( `Error fetching page ${ sourceLanguage }:${ title } for ${ targetLanguage }: ${ error.toString() }` );
			res.status( 404 )
...
function translate( req, res ) {
	...
	if ( provider ) {
		mtClient = v1Routes.getMTClient( req, res );
		if ( !mtClient ) {
			return;
		}
	}
 ...
 */
async function suggestSource( req, res ) {
	...
	const mtProviderFactory = ( from, to ) => {
		...
		const mtClient = new mtClients[ provider ]( req.app );
	...
	try {
		...
		return res.status( 500 ).send( `Error suggesting source for title "${ req.params.title }" in language "${ req.params.to }"` );
	}
}
...
async function suggestSections( req, res ) {
	...
	try {
		...
		return res.status( 500 ).send( `Error suggesting sections for title "${ sourceTitle }" in language "${ sourceLanguage }" to "${ targetLanguage }"` );
	}
}
