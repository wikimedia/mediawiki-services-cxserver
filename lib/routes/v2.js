...
class RoutesV2 extends RoutesV1 {
	...
	async suggestTargetTitle( req, res ) {
		...
		try {
			...
			this.app.logger.error( `Suggesting target title for source title "${ title }" failed for language pair ${ sourceLanguage }-${ targetLanguage }: ${ error.stack }` );
	...
	async suggestSectionsBySourceSectionTitles( req, res ) {
		...
		try {
			...
			this.app.logger.error( `Error suggesting target sections by source section titles for language pair ${ sourceLanguage }-${ targetLanguage }: ${ error.stack }` );
	...
	fetchPage( req, res ) {
		...
		return pageLoader.getPage( title, revision, true /* wrapSections */ ).then(
			...
			( error ) => {
				this.app.logger.error( `Error fetching page ${ sourceLanguage }:${ title } for ${ targetLanguage }: ${ error.toString() }` );
				res.status( 404 )
		...
		return machineTranslationRequest.then( ( translatedHTML ) => {
			...
			return new CXAdapter( from, to, api, this.app )
				...
				.then( ( adaptedDoc ) => {
					...
					this.app.logger.error( `Adaptation error for content: ${ from } > ${ to }. ` +
						`${ error.stack }` );
				} );
		}, ( error ) => {
			res.status( error.status || 500 ).end( error.stack );
			...
			this.app.logger.error( `MT processing error for: ${ from } > ${ to }. ` +
				`${ error.stack }` );
		} );
	}
	...
	async suggestSource( req, res ) {
		...
		try {
			...
			this.app.logger.error( `Error suggesting source for title "${ req.params.title }" in language "${ req.params.to }": ${ error.stack }` );
	...
	async suggestSections( req, res ) {
		...
		try {
			...
			this.app.logger.error( `Error suggesting sections for title "${ sourceTitle }" in language "${ sourceLanguage }" to "${ targetLanguage }": ${ error.stack }` );
