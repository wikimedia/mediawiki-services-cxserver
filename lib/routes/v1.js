...
class RoutesV1 {
	...
	machineTranslate( req, res ) {
		...
		return mtClient.translate( from, to, sourceHtml ).then(
			( data ) => {
				...
				this.app.metrics.increment( `translate.${req.params.provider}.200` );
			},
			( error ) => {
				res.status( 500 ).end( error.toString() );
				this.app.metrics.increment( `translate.${req.params.provider}.500` );
	...
	dictionary( req, res ) {
		...
		const provider = this.registry.getValidProvider( from, to, 'dictionary', req.params.provider );
		...
		const dictClients = require( __dirname + '/../dictionary/' );
		const dictClient = new dictClients[ provider ]( this.app );
