	...
	fetchPage( req, res ) {
		...
		return pageloader.load( title, sourceLanguage, revision ).then(
			( response ) => {
				...
				try {
					this.app.logger.log( 'debug', 'Fetch page', {
						title: title,
					...
					this.app.logger.log( 'debug', 'Segment page', {
						title: title,
						sourceLanguage: sourceLanguage
					} );
				} catch ( error ) {
					res.status( 500 ).end( `Page ${sourceLanguage} : ${title} could not be fetched or segmented: ` + error.toString() );
				}
				res.send( {
					sourceLanguage: sourceLanguage,
					title: title,
