 ...
 * @param {Request} req request object
 * @return {MTClient}
 * @throws {MTClientError}
 */
async function getMTClient( req ) {
	...
	if ( !provider ) {
		throw new MTClientError( 'Provider not supported', 404 );
	}
	...
	try {
		...
		throw new MTClientError( `MT Provider ${ provider } not found`, 500 );
	}
	...
	if ( mtClient.requiresAuthorization() ) {
		if ( !req.headers || !req.headers.authorization ) {
			throw new MTClientError( 'Authorization header is missing', 403 );
		}
		...
		try {
			...
			throw new MTClientError( `Invalid authorization header: ${ err.message }`, 403 );
		}
	}
	return mtClient;
}
...
async function machineTranslate( req, res, next ) {
	...
	// No MT client was found via provider param, validate language codes
	if ( !mtClient && !assertValidLanguageCodes( res, from, to ) ) {
		return;
	}

	const provider = mtClient.constructor.name;
	...
	if ( !sourceHtml || sourceHtml.trim().length === 0 ) {
		return next( Object.assign( new Error( 'Content for machine translation is not given or is empty' ), { status: 400 } ) );
	}
...
function listToolForLanguagePair( req, res ) {
	...
	if ( !assertValidLanguageCodes( res, from, to ) ) {
		return;
	}
